...
def getPlans(sql, hint, verbose=False, check_hint_used=False, ENABLE_LEON=False, curr_file=None):
    with pg_executor.Cursor() as cursor:
        ...
        # When leon is enabled, we need to set the query name.
        # When query name is set, leon will only consider
        # the query with the same name.
        # This is useful when we want to make sure the eqsets are consecutive.
        if ENABLE_LEON:
